...
# TODO: For initial demo only, delete at later point for more general methods below
# Overwritten generator:
class CIFAR100CL(CIFAR100):
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        """Overwritten from orgiinal torchvision loader to return indices.
    ...
    def __call__(self, imgs):
        ...
        with torch.no_grad():
            if n_blur == bsz:
                ...
                imgs_1, imgs_2 = torch.split(
                    imgs, [n_blur, bsz - n_blur], dim=0)
...
def build_transforms(aug, modal, use_memory_bank=True):
    ...
    if modal == 'RGB':
        ...
        raise NotImplementedError('No support for non image modalities yet.')
